    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="search"
            placeholder="Buscar funções cadastradas..."
            value={search}
      ...
      {showAdd && (
        <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 animate-in fade-in slide-in-from-top-2">
          <label className="block text-xs font-semibold text-blue-800 uppercase tracking-wider mb-2">Cadastrar Nova Função</label>
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleAdd}
              disabled={loadingId === 'new' || !novoCargoNome.trim() || isPending}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        {filteredCargos.length === 0 ? (
          <div className="px-6 py-12 text-center">
            ...
            <p className="text-sm font-medium text-gray-900">Nenhuma função encontrada</p>
            <p className="text-sm text-gray-500 mt-1">Crie funções para organizar melhor a sua equipe.</p>
          </div>
        ) : (
          <div className="divide-y divide-gray-100">
            {filteredCargos.map(cargo => (
              <div key={cargo.id} className="group transition-colors hover:bg-gray-50/50">
                  ...
                  {editingId !== cargo.id ? (
                    <>
                      ...
                      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity focus-within:opacity-100">
                        ...
                        <button
                          ...
                          disabled={loadingId === cargo.id || isPending}
                    ...
                    <div className="flex-1 flex items-center gap-3 animate-in fade-in">
                      <input
                        ...
                        onKeyDown={e => e.key === 'Enter' && handleSaveEdit(cargo.id)}
