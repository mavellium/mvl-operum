...
export default function ColumnHeader({ title, cardCount, onRename, onDelete }: ColumnHeaderProps) {
  const [confirmOpen, setConfirmOpen] = useState(false)

  return (
    <>
      <div className="flex items-center justify-between px-4 py-3 cursor-grab active:cursor-grabbing border-b border-black/5 group">

        {/* Lado Esquerdo: Título e Contador */}
        <div className="flex items-center gap-2.5 min-w-0">
          {onRename ? (
            ...
            <span className="font-bold text-gray-800 text-sm tracking-tight truncate">{title}</span>
          )}
        ...
        {/* Lado Direito: Botão Lixeira (apenas quando onDelete disponível) */}
        {onDelete && (
          <button
      ...
      {onDelete && (
        <ConfirmDialog
          ...
          message={`Tem certeza que deseja excluir a coluna "${title}"? Todos os cards dentro dela também serão apagados!`}
        />
      )}
    </>
  )
